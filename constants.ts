...
const generatePlayers = (baseRating: number, region: keyof typeof NATIONALITIES): Player[] => {
    ...
    return [
        { name: generateName(nats[0]), position: 'GK', rating: baseRating + 2, age: 28, nationality: nats[Math.floor(Math.random() * nats.length)], personality: 'Loyal', wage: 50000, status: { type: 'Available' }, effects: [], contractExpires: Math.floor(Math.random() * 4) + 1 },
        ...Array.from({ length: 4 }, (_, i) => ({ name: generateName(nats[0]), position: 'DEF' as const, rating: baseRating - 2 + i, age: 26+i, nationality: nats[Math.floor(Math.random() * nats.length)], personality: 'Loyal' as const, wage: 40000 + i*2000, status: { type: 'Available' } as const, effects: [], contractExpires: Math.floor(Math.random() * 4) + 1 })),
        ...Array.from({ length: 4 }, (_, i) => ({ name: generateName(nats[0]), position: 'MID' as const, rating: baseRating + i, age: 24+i, nationality: nats[Math.floor(Math.random() * nats.length)], personality: 'Ambitious' as const, wage: 60000 + i*5000, status: { type: 'Available' } as const, effects: [], contractExpires: Math.floor(Math.random() * 4) + 1 })),
        ...Array.from({ length: 2 }, (_, i) => ({ name: generateName(nats[0]), position: 'FWD' as const, rating: baseRating + 3 + i, age: 22+i, nationality: nats[Math.floor(Math.random() * nats.length)], personality: 'Ambitious' as const, wage: 80000 + i*10000, status: { type: 'Available' } as const, effects: [], contractExpires: Math.floor(Math.random() * 4) + 1 })),
...
export const TRANSFER_TARGETS: Player[] = [
    // FWD
    { name: 'Leo Messi', position: 'FWD', rating: 94, age: 39, nationality: '🇦🇷', personality: 'Ambitious', wage: 500000, status: { type: 'Available' }, effects: [], contractExpires: 1 },
    { name: 'Cristian Rolando', position: 'FWD', rating: 92, age: 41, nationality: '🇵🇹', personality: 'Mercenary', wage: 600000, status: { type: 'Available' }, effects: [], contractExpires: 1 },
    { name: 'Kylian Mbappa', position: 'FWD', rating: 93, age: 27, nationality: '🇫🇷', personality: 'Ambitious', wage: 450000, status: { type: 'Available' }, effects: [], contractExpires: 5 },
    { name: 'Eric Haaland', position: 'FWD', rating: 93, age: 26, nationality: '🇳🇴', personality: 'Ambitious', wage: 400000, status: { type: 'Available' }, effects: [], contractExpires: 4 },
    // MID
    { name: 'Luka Modric', position: 'MID', rating: 88, age: 41, nationality: '🇭🇷', personality: 'Loyal', wage: 200000, status: { type: 'Available' }, effects: [], contractExpires: 1 },
    { name: 'Kevin De Bruyne', position: 'MID', rating: 91, age: 35, nationality: '🇧🇪', personality: 'Ambitious', wage: 350000, status: { type: 'Available' }, effects: [], contractExpires: 2 },
    // DEF
    { name: 'Sergio Ramos', position: 'DEF', rating: 84, age: 40, nationality: '🇪🇸', personality: 'FiredUp' as any, wage: 100000, status: { type: 'Available' }, effects: [], contractExpires: 1 },
    { name: 'Trent A-A', position: 'DEF', rating: 87, age: 28, nationality: '🏴󠁧󠁢󠁥󠁮󠁧󠁿', personality: 'Loyal', wage: 180000, status: { type: 'Available' }, effects: [], contractExpires: 3 },
    // GK
    { name: 'Manuel Neuer', position: 'GK', rating: 86, age: 40, nationality: '🇩🇪', personality: 'Loyal', wage: 150000, status: { type: 'Available' }, effects: [], contractExpires: 1 },
    { name: 'Thibaut Courtois', position: 'GK', rating: 90, age: 34, nationality: '🇧🇪', personality: 'Ambitious', wage: 250000, status: { type: 'Available' }, effects: [], contractExpires: 3 },
];
